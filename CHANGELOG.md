...
### 修复
- 修复登录自建 Memos 服务端后一段时间被自动退出登录的问题：会话按服务器默认时长签发（约 7 天，适配 Memos 0.22–0.29），到期后自动用已保存的账号密码静默重登续期，不再签发永不过期的会话
- Token 失效时优先使用已保存的账号密码静默重登（带 5 分钟冷却防循环），避免用户被打断
- 服务器暂时离线、DNS 失败、连接超时、反向代理 5xx/限流等暂时性故障不再清除本地登录凭据，恢复后自动继续同步；只有服务器明确拒绝凭据（如密码已修改）才会要求重新登录
- 确认凭据失效后的清理更彻底：不再出现旧 Token 被写回、残留「看似已登录」的僵尸状态；删除账号/清除本机数据时同时清除已保存的密码；凭据被服务器拒绝时保存的账号密码一并清除，登录页不再预填已知失效的密码
- 设置页关闭「记住密码」会立即删除已保存的账号密码（当前会话不受影响），此后服务器端会话到期需手动重新登录
- 「API 服务初始化失败」不再直接强制登出，改为尝试恢复或回落本地模式

### 变更
- 应用完全自托管化：移除对原官方服务器（memos.didichou.site / api.didichou.site）的全部依赖
- 移除星河云验证、云公告（登录页跑马灯、通知中心云公告）和云端更新检查/强更弹窗
- 登录、注册页移除「官方服务器（推荐）」选项，改为直接填写自建 Memos 服务器地址并记住上次输入
- 通知中心专注本地提醒通知；账号删除页、隐私政策与用户协议同步改为自托管表述（法律文档版本更新为 2026-09-07）
- 移除维护 CLI 与打包脚本中的云验证密钥注入；离线图片缓存查找不再硬编码官方域名
