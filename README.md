...
本版本修复连接自建 Memos 服务端一段时间后自动退出登录的问题：会话按服务器默认时长签发，到期后自动用已保存的账号密码静默重登续期；服务器暂时不可用（离线、超时、反向代理 5xx 等）不再清除本地凭据。同时应用完全自托管化，移除对原官方服务器的依赖（云验证、云公告、云端更新检查均已下线），登录/注册页改为直接填写自建服务器地址。

主要变化：

- 会话到期后自动用已保存的账号密码静默重登，修复一段时间后被自动退出登录的问题。
